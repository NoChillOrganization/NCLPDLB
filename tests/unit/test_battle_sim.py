...
async def test_parse_replay_command_fetches_json():
    ...
    with (
        patch("src.services.battle_sim.sheets") as mock_sheets,
...
def test_matchup_score_speed_bonus():
    ...
    fast = Pokemon(
        ...
        name="Fast",
        types=["normal"],
        base_stats=PokemonStats(hp=80, atk=80, def_=80, spa=80, spd=80, spe=130),
        generation=1,
    )
    slow = Pokemon(
        national_dex=2,
        name="Slow",
        types=["normal"],
        base_stats=PokemonStats(hp=80, atk=80, def_=80, spa=80, spd=80, spe=30),
        generation=1,
    )
...
async def test_parse_replay_http_error_status():
    ...
    with patch("aiohttp.ClientSession") as MockSession:
        ...
        result = await svc.parse_replay(
            "g1", "p1", "https://replay.pokemonshowdown.com/test"
        )
...
async def test_parse_replay_non_https_url():
    ...
    with (
        patch("src.services.battle_sim.sheets"),
        patch("aiohttp.ClientSession") as MockSession,
    ):
        mock_session = AsyncMock()
        MockSession.return_value.__aenter__ = AsyncMock(return_value=mock_session)
        ...
        result = await svc.parse_replay(
            "g1", "p1", "replay.pokemonshowdown.com/gen9ou-99"
        )
...
async def test_parse_replay_parse_exception():
    ...
    with (
        patch.object(svc, "_parse_replay_data", side_effect=RuntimeError("bad data")),
        patch("aiohttp.ClientSession") as MockSession,
    ):
        mock_session = AsyncMock()
        MockSession.return_value.__aenter__ = AsyncMock(return_value=mock_session)
