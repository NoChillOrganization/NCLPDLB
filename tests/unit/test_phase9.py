...
def test_csv_drift_check():
    ...
    assert drift == {"undocumented_cmd"}, (
        f"Expected {{'undocumented_cmd'}}, got {drift}"
    )
...
def test_csv_new_row_picked_up():
    ...
    assert len(drift_after) == 0, (
        f"Expected empty drift after CSV update, got {drift_after}"
    )
...
def test_team_import_txt_parse():
    ...
    assert "Corviknight" in result, (
        "Decoded text should contain the second Pokemon name"
    )
...
def test_format_autocomplete():
    ...
    10 Smogon Gen 9 tiers (incl. Monotype Random Battle), 7 VGC regulations,
    2 Champions VGC formats, and Draft League.
    """
    ...
    vgc = [k for k in SUPPORTED_FORMATS if "vgc" in k and "champions" not in k]
    champions = [k for k in SUPPORTED_FORMATS if "champions" in k]
    draft = [k for k in SUPPORTED_FORMATS if k == "draftleague"]
    assert len(smogon) == 10, f"Expected 10 Smogon formats, got {len(smogon)}"
...
def test_team_import_confirmation_flow():
    ...
    assert pokemon_fields, (
        f"Embed must have a 'Pokemon' field, got fields: {[f.name for f in embed.fields]}"
    )
...
def test_per_format_storage():
    ...
    assert key_ou != key_uu, (
        f"gen9ou and gen9uu keys must differ: {key_ou!r} == {key_uu!r}"
    )
    assert key_ou != key_legacy, (
        f"format key must differ from legacy key: {key_ou!r} == {key_legacy!r}"
    )
    assert key_uu != key_legacy, (
        f"format key must differ from legacy key: {key_uu!r} == {key_legacy!r}"
    )
    assert key_legacy == "guild1:player1", (
        f"Legacy key format unchanged: {key_legacy!r}"
    )
