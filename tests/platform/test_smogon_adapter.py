...
async def test_fetch_from_fixture():
    ...
    with patch("src.platform.sources.smogon.get_json", new=AsyncMock(return_value=data)):
        records = list(await SmogonAdapter().fetch(
            period="2026-05", formats=["gen9vgc2025regi"], cutoff=1500,
        ))
...
async def test_fetch_multiple_formats():
    ...
    with patch("src.platform.sources.smogon.get_json", new=AsyncMock(return_value=data)):
        records = list(await SmogonAdapter().fetch(
            period="2026-05", formats=["gen9vgc2025regi", "gen9ou"], cutoff=1500,
        ))
...
async def test_fetch_skips_missing_format():
    ...
    with patch("src.platform.sources.smogon.get_json", new=AsyncMock(return_value=None)):
        records = list(await SmogonAdapter().fetch(
            period="2026-05", formats=["gen9vgc2025regi"], cutoff=1500,
        ))
    assert records == []
...
async def test_fetch_skips_malformed_response():
    ...
    with patch("src.platform.sources.smogon.get_json", new=AsyncMock(return_value=bad_data)):
        records = list(await SmogonAdapter().fetch(
            period="2026-05", formats=["gen9vgc2025regi"], cutoff=1500,
        ))
    assert records == []
