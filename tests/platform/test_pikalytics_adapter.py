...
async def test_fetch_single_page_from_fixture():
    ...
    with patch("src.platform.sources.pikalytics.get_json",
               new=AsyncMock(side_effect=[page_data, None])):
        records = list(await PikalyticsAdapter().fetch(
            formats=["gen9vgc2025regi"], max_pages=2,
        ))
...
async def test_fetch_paginates_across_pages():
    ...
    with patch("src.platform.sources.pikalytics.get_json",
        ...
        records = list(await PikalyticsAdapter().fetch(
            formats=["gen9vgc2025regi"], max_pages=3,
        ))
    assert len(records) == 1
...
async def test_fetch_multiple_formats():
    ...
    with patch("src.platform.sources.pikalytics.get_json",
        ...
        records = list(await PikalyticsAdapter().fetch(
            formats=["gen9vgc2025regi", "gen9ou"], max_pages=2,
        ))
...
async def test_fetch_empty_format_produces_no_record():
    ...
    with patch("src.platform.sources.pikalytics.get_json",
        ...
        records = list(await PikalyticsAdapter().fetch(
            formats=["unknown-format"], max_pages=2,
        ))
    assert records == []
