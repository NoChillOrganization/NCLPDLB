    ...
    async with pool.acquire() as conn:
        for record in records:
            raw_id = await land_raw(
                conn, source="showdown", route=record.route,
                natural_key=record.natural_key, payload=record.payload, url=record.url,
            )
            if raw_id is None:
                continue  # identical payload already landed
            await normalize_replay_row(conn, raw_id=raw_id, source="showdown", payload=record.payload)
        ...
        for adapter in (SmogonAdapter(), PikalyticsAdapter()):
            ...
            for record in records:
                ...
                await normalize_usage_row(
                    conn, raw_id=raw_id, source=adapter.source,
                    natural_key=record.natural_key, payload=record.payload,
                )
                normalized += 1
    ...
    async with pool.acquire() as conn:
        ...
        for record in records:
            ...
            await normalize_tournament_row(
                conn, raw_id=raw_id, source="limitless",
                natural_key=record.natural_key, payload=record.payload,
            )
            normalized += 1
    print(f"landed={landed} normalized={normalized}")
...
def main() -> None:
    ...
    usage_parser.add_argument("--period", required=True, help="YYYY-MM, used by Smogon only")
    ...
    event_parser.add_argument("--ids", nargs="+", help="specific Limitless tournament ids")
