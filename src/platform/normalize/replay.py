...
async def normalize_replay_row(conn: asyncpg.Connection, *, raw_id: int, source: str, payload: dict) -> int:
    ...
    replay_db_id = await upsert_replay(
        conn, source=source, replay_id=record.replay_id, format_id=None,
        players={"p1": record.p1_name, "p2": record.p2_name}, rating=record.rating,
        log_hash=log_hash, raw_ingest_id=raw_id,
    )

    battle_id = await upsert_replay_battle(
        conn, replay_db_id=replay_db_id, winner=record.winner, turn_count=record.total_turns,
        turns=record.to_dict()["turns"], parser_version=PARSER_VERSION,
    )
    ...
    for player_slot, team, active in (
        ...
        for species in team:
            ...
            await add_replay_team_member(
                conn, replay_battle_id=battle_id, player_slot=player_slot,
                canonical_species_id=species_id, brought=True, lead=(species == active),
            )
