...
async def normalize_usage_row(conn, *, raw_id: int, source: str, natural_key: str, payload: dict) -> int:
    if source == "smogon":
        snapshot_id = await _from_smogon(conn, source=source, natural_key=natural_key, payload=payload, raw_id=raw_id)
    elif source == "pikalytics":
        snapshot_id = await _from_pikalytics(conn, source=source, payload=payload, raw_id=raw_id)
    else:
...
async def _from_smogon(conn, *, source: str, natural_key: str, payload: dict, raw_id: int) -> int:
    ...
    format_id = await upsert_canonical_format(
        conn, slug=fmt_slug, label=fmt_slug.upper(), generation=gen, game_type=game_type,
    )
    info = payload.get("info", {})
    snapshot_id = await upsert_usage_snapshot(
        conn, source=source, format_id=format_id, period=f"{period_str}-01",
        elo_cutoff=int(cutoff_str), sample_size=info.get("number of battles"), raw_ingest_id=raw_id,
    )

    species_data = payload.get("data", {})
    ranked = sorted(species_data.items(), key=lambda kv: kv[1].get("usage", 0), reverse=True)
    for rank, (species_name, stats) in enumerate(ranked, start=1):
        species_id = await resolve_species(
            conn, source=source, raw_name=species_name, normalized_key=_normalized_key(species_name),
        )
        entry_id = await upsert_usage_entry(
            conn, snapshot_id=snapshot_id, canonical_species_id=species_id, rank=rank,
            usage_pct=stats.get("usage"), raw_count=stats.get("Raw count"),
        )
        await upsert_usage_moveset(
            conn, usage_entry_id=entry_id, moves=stats.get("Moves", {}), items=stats.get("Items", {}),
            spreads=stats.get("Spreads", {}), abilities=stats.get("Abilities", {}),
            teammates=stats.get("Teammates", {}), checks=stats.get("Checks and Counters", {}),
        )
    return snapshot_id
...
async def _from_pikalytics(conn, *, source: str, payload: dict, raw_id: int) -> int:
    ...
    snapshot_id = await upsert_usage_snapshot(
        conn, source=source, format_id=format_id, period=date.today(), elo_cutoff=None,
        sample_size=len(entries) or None, raw_ingest_id=raw_id,
    )
    for rank, entry in enumerate(entries, start=1):
        ...
        entry_id = await upsert_usage_entry(
            conn, snapshot_id=snapshot_id, canonical_species_id=species_id, rank=rank,
            usage_pct=entry.get("usage"), raw_count=entry.get("raw_count"),
        )
        await upsert_usage_moveset(
            conn, usage_entry_id=entry_id, moves=entry.get("moves", {}), items=entry.get("items", {}),
            spreads=entry.get("spreads", {}), abilities=entry.get("abilities", {}),
            teammates=entry.get("teammates", {}), checks=entry.get("checks", {}),
        )
    return snapshot_id
