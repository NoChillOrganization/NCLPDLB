...
class ScheduleAddModal(discord.ui.Modal, title="Add Schedule Match"):
    ...
    pool = discord.ui.TextInput(
        label="Pool (A or B)", placeholder="A", max_length=1, default="A"
    )
    ...
    game_fmt = discord.ui.TextInput(
        label="Game Format (showdown/sv/vgc…)",
...
class ResultSetModal(discord.ui.Modal, title="Record Match Result"):
    match_id = discord.ui.TextInput(
        label="Match ID (leave blank = latest)", max_length=20, required=False
    )
    winner_name = discord.ui.TextInput(label="Winner Name", max_length=60)
    replay_url = discord.ui.TextInput(
        label="Replay URL (optional)", max_length=300, required=False
    )
    video_url = discord.ui.TextInput(
        label="Video URL (optional)", max_length=300, required=False
    )
...
class TransactionModal(discord.ui.Modal, title="Log Transaction"):
    txn_type = discord.ui.TextInput(
        label="Type (trade/drop/add/waiver)", placeholder="trade", max_length=20
    )
...
class RuleAddModal(discord.ui.Modal, title="Add Rule"):
    category = discord.ui.TextInput(
        label="Category (e.g. Tera Captains, Trading)", max_length=60
    )
    title_ = discord.ui.TextInput(label="Rule Title", max_length=80)
    description = discord.ui.TextInput(
        label="Rule Description", style=discord.TextStyle.paragraph, max_length=1000
    )
...
class SetupEditModal(discord.ui.Modal, title="Edit Setup Value"):
    field_name = discord.ui.TextInput(
        label="Field Name (see /sheet-setup view)", max_length=60
    )
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        if not row:
            await interaction.followup.send(
                "❌ No setup found for this server.", ephemeral=True
            )
            return
...
class PlayerTeamModal(discord.ui.Modal, title="Update Player Team"):
    ...
    logo_url = discord.ui.TextInput(
        label="Logo URL (optional)", max_length=300, required=False
    )
...
class PlayoffAddModal(discord.ui.Modal, title="Add Playoff Match"):
    ...
    pool = discord.ui.TextInput(
        label="Pool (A/B or Finals)", max_length=10, default="A"
    )
    p1 = discord.ui.TextInput(
        label="Player 1 Name · Seed", placeholder="TravisT · 1", max_length=60
    )
    p2 = discord.ui.TextInput(
        label="Player 2 Name · Seed", placeholder="Rival · 2", max_length=60
    )
    winner = discord.ui.TextInput(
        label="Winner Name (leave blank if not played)", required=False, max_length=60
    )
...
class SheetCog(commands.Cog, name="Sheet"):
    ...
    @app_commands.describe(
        pool="Filter to a specific pool (A or B), or leave blank for all"
    )
    async def sheet_standings(
        self, interaction: discord.Interaction, pool: str = ""
    ) -> None:
        await interaction.response.defer(ephemeral=True)
    ...
    # ── /sheet-schedule add ───────────────────────────────────
    @app_commands.command(
        name="sheet-schedule", description="Add a match to the Schedule tab"
    )
    @require_role(ROLE_MOD)
    ...
    # ── /sheet-result set ─────────────────────────────────────
    @app_commands.command(
        name="sheet-result", description="Record a match result in the spreadsheet"
    )
    @require_role(ROLE_MOD)
    ...
    # ── /sheet-transaction ────────────────────────────────────
    @app_commands.command(
        name="sheet-transaction",
        description="Log a trade, drop, or add to Transactions tab",
    )
    @require_role(ROLE_MOD)
    ...
    # ── /sheet-setup view ─────────────────────────────────────
    @app_commands.command(
        name="sheet-setup", description="View or edit the league Setup tab"
    )
    ...
    async def sheet_setup(
        self, interaction: discord.Interaction, action: str = "view"
    ) -> None:
        if action == "edit":
        ...
        row = await asyncio.to_thread(
            sheets.get_league_setup, str(interaction.guild_id)
        )
        if not row:
            await interaction.followup.send(
                "❌ No setup found. Run `/draft-setup` first.", ephemeral=True
            )
            return
    ...
    @app_commands.command(
        name="sheet-player",
        description="Update a player's team name, pool, or logo in the spreadsheet",
    )
    @require_role(ROLE_MOD)
    ...
    @app_commands.command(
        name="sheet-pokedex",
        description="Sync local Pokemon data to the Pokedex tab (slow)",
    )
    @require_role(ROLE_GUILDMASTER)
    ...
    # ── /sheet-playoff add ────────────────────────────────────
    @app_commands.command(
        name="sheet-playoff", description="Add a playoff match to the Playoffs tab"
    )
    @require_role(ROLE_MOD)
